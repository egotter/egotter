...
describe StopAllReportsResponder::Processor do
  let(:uid) { 1 }
  let(:text) { 'text' }
  ...
  describe '#send_message' do
    ...
    it do
      expect(StopAllReportsWorker).to receive(:perform_async).with(user.id)
      expect(CreateAllReportsStoppedMessageWorker).to receive(:perform_async).with(user.id)
      subject
    end
  end
end
