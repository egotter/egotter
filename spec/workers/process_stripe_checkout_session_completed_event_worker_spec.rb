...
RSpec.describe ProcessStripeCheckoutSessionCompletedEventWorker do
  ...
  before do
    allow(worker).to receive(:send_message).with(:orders_cs_completed, an_instance_of(String), checkout_session_id, user_id: user.id, order_id: order.id, order_name: order.name)
  end
