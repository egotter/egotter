...
RSpec.describe CreateFollowLimitation, type: :model do
  context 'Constants' do
    it do
      expect(described_class::ANONYMOUS).to eq(2)
      expect(described_class::BASIC_PLAN).to eq(100)
    end
  end
  ...
  describe '.max_count' do
    ...
    context 'User is passed' do
      ...
      context 'user#has_valid_subscription? == true' do
        let(:value) { true }
        before do
          allow(TwitterUser).to receive_message_chain(:latest_by, :followers_count).with(uid: user.uid).with(no_args).and_return(100)
        end
  ...
  describe '.follow_requests' do
    ...
    let!(:request1) { user.follow_requests.create!(uid: 1, created_at: 2.hour.ago) }
    let!(:request2) { user.follow_requests.create!(uid: 2, created_at: 1.hour.ago) }
    let!(:request3) { user.follow_requests.create!(uid: User::EGOTTER_UID) }
    subject { described_class.follow_requests(user) }
    it { is_expected.to match_array([request1, request2]) }
  end

  describe '.current_count' do
    let(:user) { instance_double(User) }
    subject { described_class.current_count(user) }
    before { allow(described_class).to receive(:follow_requests).with(user).and_return([1, 2, 3]) }
  ...
  describe '.count_reset_in' do
    ...
    before do
      request = user.follow_requests.create!(uid: 1, created_at: 1.hour.ago)
