...
class UsageStat < ApplicationRecord
  ...
  class TweetCluster
    ...
    def count_words(text, limit: 100, allow_list: ALLOW_LIST, deny_set: DENY_SET, min_word_length: 3, min_count: 2)
      ...
      text.scan(SPLIT_REGEXP).each do |word|
        word = word.split(PUNCTUATION_REGEXP).join('')
        word = word.split(CHOONPU_REGEXP).join('')

        next if deny_set[word] || word.length < min_word_length
        map[word] += 1
      end

      map.delete_if { |_, count| count < min_count }.sort_by { |_, count| -count }.take(limit).to_h
    end
  end
