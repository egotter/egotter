...
module Concerns::TwitterUser::Persistence
  ...
  def import_twitter_db_users
    self.class.benchmark_and_silence(:twitter_db_users) do
      TwitterDB2::User.import_from!(friends)
      TwitterDB2::User.import_from!(followers)
      TwitterDB2::User.import_from!([self])
    end
  rescue => e
    logger.warn "#{self.class}##{__method__}: #{e.class} #{e.message} #{self.inspect}"
    false
  ...
  def import_relationships
    self.class.benchmark_and_silence(:twitter_db_users) do
      ActiveRecord::Base.transaction do
        TwitterDB2::Friendship.import_from!(self.uid, friend_uids)
        TwitterDB2::Followership.import_from!(self.uid, follower_uids)
        ...
        TwitterDB2::User.find_by(uid: self.uid).update_columns(friends_size: friend_uids.size, followers_size: follower_uids.size)
      end
    end
  rescue => e
    logger.warn "#{self.class}##{__method__}: #{e.class} #{e.message} #{self.inspect}"
