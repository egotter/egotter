...
class Unfriendship < ActiveRecord::Base
  ...
  def self.import_from!(twitter_user)
    unfriendships = twitter_user.calc_removing.map.with_index { |u, i| [u.id, twitter_user.uid.to_i, i] }

    ActiveRecord::Base.transaction do
      delete_all(from_uid: twitter_user.uid)
      import(%i(friend_id from_uid sequence), unfriendships, validate: false, timestamps: false)
    end
  end
end
