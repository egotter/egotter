...
module Concerns::ExceptionHandler
  ...
  def not_found_message(screen_name)
    if user_signed_in?
      t('after_sign_in.not_found_html', user: view_context.user_link(screen_name))
    else
      t('before_sign_in.not_found_html', user: view_context.user_link(screen_name), url: kick_out_error_path('not_found'))
    end
  end

  def forbidden_message(screen_name)
    if user_signed_in?
      t('after_sign_in.forbidden_html', user: view_context.user_link(screen_name))
    else
      t('before_sign_in.forbidden_html', user: view_context.user_link(screen_name), url: kick_out_error_path('forbidden'))
    end
  end
  ...
  def suspended_message(screen_name)
    if user_signed_in?
      t('after_sign_in.suspended_html', user: view_context.user_link(screen_name))
    else
      t('before_sign_in.suspended_html', user: view_context.user_link(screen_name), url: kick_out_error_path('suspended'))
    end
  end

  def protected_message(screen_name)
    if user_signed_in?
      t('after_sign_in.protected_html', user: view_context.user_link(screen_name), url: kick_out_error_path('protected'))
    else
      t('before_sign_in.protected_html', user: view_context.user_link(screen_name), url: kick_out_error_path('protected'))
    end
  end

  def blocked_message(screen_name)
    if user_signed_in?
      t('after_sign_in.blocked_html', user: view_context.user_link(screen_name))
    else
      raise
    end
  end
