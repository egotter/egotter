module Validation
  ...
  def need_login
    redirect_to_root t('before_sign_in.need_login', sign_in_link: view_context.link_to(t('dictionary.sign_in'), welcome_path)) unless user_signed_in?
  end
  ...
  def valid_search_value?
    ...
    if tu.suspended_account?(twitter_link)
      return redirect_to_root tu.errors[:base].join(t('dictionary.delim'))
    end
    ...
    redirect_to_root t('before_sign_in.too_many_requests', sign_in_link: view_context.link_to(t('dictionary.sign_in'), welcome_path))
  rescue Twitter::Error::NotFound => e
    redirect_to_root t('before_sign_in.not_found')
    ...
    redirect_to_root alert_msg.html_safe
  rescue Twitter::Error::Forbidden => e
    redirect_to_root t('before_sign_in.forbidden')
  rescue => e
    logger.warn "#{self.class}##{__method__} #{e.class} #{e.message} #{params[:screen_name]}"
    redirect_to_root t('before_sign_in.something_is_wrong', sign_in_link: view_context.link_to(t('dictionary.sign_in'), welcome_path))
  end

  private

  def redirect_to_root(alert)
