...
module SearchRequestConcern
  ...
  included do
    ...
    before_action(only: :show) { !@self_search && not_found_twitter_user?(params[:screen_name]) }
    before_action(only: :show) { !@self_search && forbidden_twitter_user?(params[:screen_name]) }
    ...
    before_action(only: :show) { too_many_friends?(@twitter_user) }
