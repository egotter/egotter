class TwitterUsersController < ApplicationController
  ...
  def create_changes_text(twitter_user)
    ...
    logger.warn "#{controller_name}##{__method__} The request has timed out. (#{seconds.inspect}) #{e.class} #{e.message} #{twitter_user.id}"
  ...
  class ChangesTextBuilder
    ...
    def build
      ...
      logger.warn "#{self.class}##{__method__} #{e.inspect} #{previous_twitter_user&.id} #{current_twitter_user.id}"
      logger.info e.backtrace.join("\n")
