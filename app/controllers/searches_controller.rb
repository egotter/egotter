class SearchesController < ApplicationController
  ...
  def current_visit_id
    if current_visit.nil?
      ...
      begin
        track_ahoy_visit
        logger.info "[debug ahoy] call track_ahoy_visit #{current_visit.inspect} #{Ahoy.server_side_visits} #{Ahoy.cookies} #{Ahoy.server_side_visits != true && !Ahoy.cookies} #{ahoy.new_visit?} #{ahoy.send(:exclude?)} #{ahoy.missing_params?} #{DeviceDetector.new(request.user_agent).device_type}"
      rescue => e
