class ApplicationController < ActionController::Base
  ...
  rescue_from ActionController::InvalidAuthenticityToken do |e|
    if request.referer.present? && !request.referer.match(%r{^https://egotter.com})
      screen_name = params[:screen_name].strip.remove /^@/

      if screen_name&.match(Validations::ScreenNameValidator::REGEXP) && controller_name == 'searches' && action_name == 'create'
        logger.warn "cross domain post #{request.device_type} #{current_user_id} #{request.fullpath} #{request.referer} #{params['screen_name']}"
        _search_path = search_path(screen_name: screen_name)
        _sign_in_path = sign_in_path via: "#{controller_name}/#{action_name}/cross_domain_post_and_recover", redirect_path: _search_path
        redirect_to root_path, alert: t('before_sign_in.cross_domain_post_and_recover_html', user: screen_name, search_path: _search_path, sign_in_path: _sign_in_path)
      else
  ...
  def sanitized_redirect_path(path)
    regexp = %r{^/(one_sided_friends|unfriends|inactive_friends|friends|conversations|clusters|searches|timelines|close_friends)}
