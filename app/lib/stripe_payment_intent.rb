class StripePaymentIntent
  ...
  class << self
    ...
    def intent_for_bank_transfer?(intent)
      ...
      Airbag.warn "#{__method__}: payment_intent_id=#{intent.id} exception=#{e.inspect}"
      Airbag.warn "#{__method__}: payment_method_types=#{intent.payment_method_types.inspect} payment_method_options=#{intent.payment_method_options.inspect}"
      false
    end
