...
class ImportTwitterDBUserWorker
  ...
  def perform(data, options = {})
    ...
    ImportTwitterDBUserForRetryingDeadlockWorker.perform_in(rand(20) + 15, data, options)
    Airbag.info "#{e.class} found", options: options
    SendMessageToSlackWorker.perform_async(:job_deadlock, "#{self.class}##{__method__}: #{options}")
  rescue => e
