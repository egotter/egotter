class CreateUnfollowWorker
  ...
  def perform(request_id, options = {})
    ...
    Airbag.warn "#{e.class} Retry later"
    CreateUnfollowWorker.perform_in(retry_in, request_id, options)
    ...
    Airbag.warn "Don't care. #{e.inspect}"

  rescue => e
    Airbag.warn "Don't retry. #{e.class} #{e.message} #{request_id} #{options.inspect} #{"Caused by #{e.cause.inspect}" if e.cause}"
