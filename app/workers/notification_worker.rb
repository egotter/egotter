class NotificationWorker
  ...
  def perform(user_id, options = {})
    ...
    if options[:type] == BACKGROUND_SEARCH && user.notification.can_send_search?
      ...
      text = I18n.t('dictionary.your_account_is_updated', kaomoji: Kaomoji.happy, url: "http://egotter.com/searches/#{user.uid}?screen_name=#{user.screen_name}", menu_url: 'http://egotter.com/menu')
      client.create_direct_message(user.uid.to_i, text)
