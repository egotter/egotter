namespace :favorite_friendships do
  desc 'update'
  task update: :environment do
    ...
    do_import = lambda do |total_uids|
      ...
      begin
        ...
        if e.message == 'No user matches for specified terms.'
          ...
          File.write('favorite_friendships_update_uids.txt', candidate_uids.join("\n"))
          raise
        end
      end
    ...
    total_favorite_friend_uids = []

    uids.each do |uid|
      next if FavoriteFriendship.where(from_uid: uid).any?

      twitter_user = TwitterUser.latest(uid)

      favorite_friend_uids = twitter_user.calc_favorite_friend_uids
      FavoriteFriendship.import_from!(uid, favorite_friend_uids)
      total_favorite_friend_uids.concat favorite_friend_uids

      if total_favorite_friend_uids.size > 1000
        do_import.call(total_favorite_friend_uids)
        total_favorite_friend_uids = []
      end
    ...
    if total_favorite_friend_uids.any?
      do_import.call(total_favorite_friend_uids)
    end
