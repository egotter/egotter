namespace :inactive_friendships do
  desc 'refresh inactive_friendships, inactive_followerships and inactive_mutual_friendships'
      ...
      TwitterUser.pluck(:uid).uniq.each_slice(batch_size) do |uids|
        uids.each do |uid|
          begin
            ...
            InactiveFriendship.import_from!(twitter_user.uid, twitter_user.calc_inactive_friend_uids)
            InactiveFollowership.import_from!(twitter_user.uid, twitter_user.calc_inactive_follower_uids)
            InactiveMutualFriendship.import_from!(twitter_user.uid, twitter_user.calc_inactive_mutual_friend_uids)
          rescue => e
            failed = true
