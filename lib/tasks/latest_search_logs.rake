namespace :latest_search_logs do
  ...
  task update: :environment do
    ...
    start_time = 30.days.ago.beginning_of_day.to_s(:db)
    end_time = Time.zone.now.end_of_day.to_s(:db)
    ActiveRecord::Base.connection.execute("insert into latest_search_logs select * from search_logs where created_at BETWEEN '#{start_time}' AND '#{end_time}' and device_type not in ('crawler', 'UNKNOWN', 'misc')")
  end
end
