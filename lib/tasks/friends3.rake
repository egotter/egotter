namespace :friends3 do
  desc 'reset'
  task reset: :environment do
    ActiveRecord::Base.connection.execute('DROP TABLE IF EXISTS tmp3_friends')
    ActiveRecord::Base.connection.execute('CREATE TABLE tmp3_friends like friends')
    ActiveRecord::Base.connection.execute('ALTER TABLE tmp3_friends DROP updated_at')
    ActiveRecord::Base.connection.execute('ALTER TABLE tmp3_friends ROW_FORMAT = COMPRESSED')
    ActiveRecord::Base.connection.execute('ALTER TABLE tmp3_friends KEY_BLOCK_SIZE = 4')
  end

  desc 'rename'
  task rename: :environment do
    ActiveRecord::Base.connection.execute('ALTER TABLE friends RENAME old_friends')
    ActiveRecord::Base.connection.execute('ALTER TABLE tmp3_friends RENAME friends')
  end

  desc 'drop_index'
  task drop_index: :environment do
    ActiveRecord::Base.connection.execute('ALTER TABLE tmp3_friends DROP INDEX index_friends_on_uid')
    ActiveRecord::Base.connection.execute('ALTER TABLE tmp3_friends DROP INDEX index_friends_on_from_id')
    ActiveRecord::Base.connection.execute('ALTER TABLE tmp3_friends DROP INDEX index_friends_on_screen_name')
    ActiveRecord::Base.connection.execute('ALTER TABLE tmp3_friends DROP INDEX index_friends_on_created_at')
  end

  desc 'add_index'
  task add_index: :environment do
    ActiveRecord::Base.connection.execute('CREATE  INDEX `index_friends_on_uid`  ON `tmp3_friends` (`uid`)')
    ActiveRecord::Base.connection.execute('CREATE  INDEX `index_friends_on_from_id`  ON `tmp3_friends` (`from_id`)')
    ActiveRecord::Base.connection.execute('CREATE  INDEX `index_friends_on_screen_name`  ON `tmp3_friends` (`screen_name`)')
    ActiveRecord::Base.connection.execute('CREATE  INDEX `index_friends_on_created_at`  ON `tmp3_friends` (`created_at`)')
  end
  ...
  task import: :environment do
    ...
    Rails.logger.silence do
      Friend.find_in_batches(start: start, batch_size: 5000) do |friends_array|
        ...
        begin
          Tmp3Friend.import(%i(id uid screen_name user_info from_id created_at), friends, validate: false)
  ...
  task verify: :environment do
    Rails.logger.silence do
      ...
      puts "#{Tmp3Friend.table_name}:"
      puts "  id_max: #{Tmp3Friend.all.maximum(:id)}, auto_increment: #{Tmp3Friend.find_by_sql([sql, table: :tmp_friends]).first.at}, match: #{Tmp3Friend.where(id: ids).count}"
...
class Tmp3Friend < ActiveRecord::Base
end