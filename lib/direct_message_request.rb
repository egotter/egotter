...
class DirectMessageRequest
  ...
  def initialize(client, user_id, text, buttons = [])
    request_method = :json_post
    path = '/1.1/direct_messages/events/new.json'

    message_data = {text: text}
    message_data[:ctas] = [{type: "web_url", label: buttons[0][:label], url: buttons[0][:url]}] unless buttons.empty?
    event = {type: "message_create", message_create: {target: {recipient_id: user_id}, message_data: message_data}}
    options = {event: event}
